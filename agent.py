...
class Agent:
    ...
    def __init__(self):
        self.n_games = 0
        self.epsilon = 0    # randomness check
        self.gamma = 0.9      # discount rate
        self.memory = deque(maxlen=MAX_MEM)     #pop_left()
    ...
    def remember(self, state, action, reward, next_state, gameover):
        self.memory.append((state, action, reward, next_state, gameover))   # popleft if MAXMEMORY is reached
    ...
    def get_action(self, state):
        # random_moves: tradeoff exploration/exploitation
        self.epsilon = 80 - self.n_games
        final_move = [0,0,0]

        # If less number of games, then next move is random else the model predicts the next move
